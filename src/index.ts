...
> = 迭代次数 extends 棋子坐标["纵"]
  ? 小于等于<迭代次数, 最大纵坐标> extends 是
    ? 获取棋盘指定行<某棋盘, 迭代次数> extends infer 当前行
      ? 当前行 extends 棋盘行
        ? 相等<迭代次数, 行号> extends 是
          ? 构造棋盘行<{ 内容: 新行["内容"] }, 当前行["下一行"]>
          : 构造棋盘行<
              { 内容: 当前行["内容"] },
              替换棋盘某行<某棋盘, 行号, 新行, 加一<迭代次数>>
            >
        : 不可能
      : 不可能
    : 不可能
...
> = 迭代次数 extends 棋子坐标["横"]
  ? 小于等于<迭代次数, 最大横坐标> extends 是
    ? 获取某行的指定单元格<某行, 迭代次数> extends infer 当前格
      ? 当前格 extends 棋盘格
        ? 相等<迭代次数, 列号> extends 是
          ? 从内容构造棋盘格<某棋子, 当前格["下一格"]>
          : 从内容构造棋盘格<
              当前格["内容"],
              替换棋盘行某格<某行, 列号, 某棋子, 加一<迭代次数>>
            >
        : 不可能
      : 不可能
    : 不可能
...
export type 渲染棋盘<某棋盘 extends 棋盘> = {
  [key in
    ...
    | "七"
    | "八"
    | "九"]: key extends keyof 正数键值对
    ? 获取棋盘指定行<某棋盘, 正数键值对[key]> extends infer 某行
      ? 某行 extends 棋盘行
        ? `⎪ ${渲染行内容<某行["内容"], 正数键值对[key]>} ⎪`
        : 不可能
      : 不可能
    : 不可能;
};
